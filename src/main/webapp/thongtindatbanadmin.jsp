...
<nav class="navbar navbar-expand-lg navbar-light bg-light">
    ...
    <div class="collapse navbar-collapse" id="navbarNav">
        ...
        <c:if test="${not empty sessionScope.quanly or not empty sessionScope.admin}">
			<li class="nav-item">
                <a class="nav-link" href="lichsukhachdatban">Lịch sử khách đặt bàn</a>
            </li>
...
<h2>Danh Sách Bàn Cần Xác Nhận</h2>
	...
	<tr>
		...
		<td>${s.gio}</td>
		<td>${s.soluong}</td>
		<td><a href="trangxacnhandatban?makh=${s.makh}&madatban=${s.madatban}" class="btn btn-primary">Xem bàn</a></td>
		<c:if test="${not empty sessionScope.admin or not empty sessionScope.quanly }">
		<td><a href="huyban?makh=${s.makh}&madatban=${s.madatban}" class="btn btn-danger">Hủy</a></td>
		</c:if>
		</tr>
		</c:forEach>
	</table>
